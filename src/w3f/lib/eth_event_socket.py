...
class SwapData:
    ...
    def __init__(self, event_response: dict, chain=network.ChainId.ETH):
        self.chain = chain
        self.explorer = network.EXPLORER[self.chain]
        ...
        self.addr_link = f"[{self.addr[:8]}]({self.explorer.address(self.addr)})"
        self.tx = event_response['transactionHash']
        self.tx_link = f"[{self.tx[:8]}]({self.explorer.tx(self.tx)})"
    ...
    @staticmethod
    async def wait(ws, chain=network.ChainId.ETH):
        try:
