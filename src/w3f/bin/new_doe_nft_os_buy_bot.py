...
def main():
    ...
    ######## Details required from the user
    DISCORD_TOKEN = hd.DSCRD_CHAN['token']
    DSCRD_CHANS = DiscordChannels(buys_id=hd.DSCRD_CHAN.get("buys", 0),
                                  listings_id=hd.DSCRD_CHAN.get("listings", 0),
                                  other_id=hd.DSCRD_CHAN.get("other", 0),
                                  debug_id=hd.DSCRD_CHAN.get("debug", 0))
    OS_TOKEN = hd.OS_KEY
    ######## Details required from the user
    METADATA = doe_nft_data.Metadata()
